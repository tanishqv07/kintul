...
const Services = () => {
  ...
  const handleBooking = async (serviceId, serviceTitle) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {services.map((service) => (
          <div key={service._id} className="bg-white shadow-md p-4 rounded-lg">
            ...
            <button className="mt-4 px-4 py-2 bg-blue-600 text-white rounded-md" onClick={() => handleBooking(service._id, service.title)}>
              Book Now
            </button>
          </div>
        ))}
